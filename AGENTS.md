...
| `douyin-video-tool/` | 个人小工具：粘贴抖音链接 → yt-dlp 自动下载高清原片 → 本地视频库点开即播（RANGE/全屏投大屏），纯 Node 零依赖，127.0.0.1:8120，桌面「抖音视频抓取」 | ✅ 独立，2026-08-20 新建 |
...
> 5. **《我的世界》（试听课 A3 环节，模块 `A3_WORLD`）**：目标系统是 `aliyun-classroom-v2`。文档在 `aliyun-classroom-v2/docs/my-world/`（01~06 + reference）。**表现规格引擎（spec.ts/engine.ts/GET /api/world）对 A3 已废弃（2026-08-26 调查确认）**：无任何前端消费者，`api/a3/world/life/route.ts` 头注释明示"旧 WorldScreen/engine 的三倾向模型不再用于 A3"，勿再往这条链投入。**⚠️ 三端实际渲染走 iframe 静态页（2026-08-26 确认）：大屏 = `public/a3/bigscreen.html`、学生端 = `public/a3/student.html`（现役实现）；React 组件 `WorldScreen.tsx`/`WorldStudent.tsx` 全仓无人 import，是僵尸副本，已标 @deprecated，勿再改它们**。教师端「发布 Tips」链路：POST `/api/world/popup` 存状态 → bigscreen.html 每 2 秒轮询弹遮罩卡片（2026-08-26 commit `e10f72c` 修复"点 Tips 大屏无反应"）。**行为系统（Behavior v1）契约已冻结（2026-08-26），决策核已实现（2026-08-29），后端链路已接通（2026-08-29）**：让学生生命摆脱"白板粒子"——六块描述 → AI 综合行为解释器 → seek/rules/body 行为程序 → 大屏决策核执行。设计基准见 `docs/my-world/06_行为语言与综合解释器契约.md`；决策核规格与实现见 `docs/my-world/07_大屏行为决策核规格.md`（Q1-Q7 裁定 + 49 格逐格 + 盲眼测试 + 刁钻回归 + runBehavior 落地 bigscreen.html，学生生命走新核、老师生命 legacy 兜底，失败退白板）。**后端 `/api/a3/behavior` 已实现**：六块设计→behavior 说明书落库（`src/lib/world/behavior.ts` 解释器+白名单过闸+规则回退），`student.html` 提交前生成 behavior 随生命提交，大屏决策核已接通学生设计。进度见待办清单 A 节。
