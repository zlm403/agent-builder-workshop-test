...
| 端 | 目录 | 干什么 |
|---|---|---|
| 学生端 | `student/` | 三项目通用工作台：任务看板 + "我有问题"提问框（实时发给老师）+ 引导 + 作品运行容器 + "我的理解棱镜" |
...
- **实时互动**：学生端任务看板下"有问题？和老师聊"对话框（上对话记录、下输入框）→ `student_ask` 事件（带 sid/course）；教师端独立"消息中心"面板（原统计区位置）定向接收/回复：每条消息标注「学生 X → 老师」/「我 → X」，回复填目标学生上课号（点学生列表自动填入），`teacher_reply` 事件带学生 sid。双向实时、定向不串线。
