...
- **实时互动**：学生端任务看板下"有问题？和老师聊"对话框（上对话记录、下输入框）→ `student_ask` 事件（带 sid/course）；教师端独立"消息中心"面板（原统计区位置）定向接收/回复：消息按学生分组，点「回复」按钮或学生列表自动定位目标，`teacher_reply` 事件带学生 sid。双向实时、定向不串线。
