...
import { P2_STEPS, P2_BIGSCREEN_HINTS } from '@/features/siteEntry/config';
  ...
  return (
    <div style={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', gap: 24, padding: '8px 0' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 22, fontWeight: 800, color: '#38bdf8' }}>P2 · 快速入门网站</div>
          ...
          return (
            <div
              ...
              style={{
                ...
                border: lit ? '1px solid rgba(56,189,248,0.7)' : '1px solid var(--border)',
                background: lit ? 'rgba(56,189,248,0.16)' : 'rgba(15,23,42,0.4)',
                boxShadow: lit ? '0 0 22px rgba(56,189,248,0.25)' : 'none',
                transition: 'all .4s',
                opacity: lit ? 1 : 0.55,
              }}
            >
              <div style={{ fontSize: 12, color: lit ? '#38bdf8' : 'var(--muted)', fontWeight: 700 }}>{n}</div>
              <div style={{ fontSize: 15, fontWeight: 700, margin: '4px 0 6px', color: lit ? '#fff' : '#9aa4b4' }}>{s.name}</div>
              <div style={{ fontSize: 11, color: lit ? '#cdd5e3' : '#6b7280', lineHeight: 1.4 }}>{P2_BIGSCREEN_HINTS[i]}</div>
              {lit && !launched && (
      ...
      {launched ? (
        <div style={{ flex: 1, overflowY: 'auto', padding: '6px 4px' }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: '#86efac', marginBottom: 14 }}>
            {"全班入门网站 · 作品墙"} ({data?.finished ?? 0} 人已发布)
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 14 }}>
            ...
            {(data?.cols ?? []).length === 0 && (
              <div style={{ color: 'var(--muted)', fontSize: 15 }}>还没有人发布，等同学们的作品上墙…</div>
            )}
          </div>
        </div>
      ) : (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 16 }}>
          <div style={{ fontSize: 'clamp(22px,3vw,40px)', fontWeight: 800, maxWidth: 1100, textAlign: 'center', background: 'linear-gradient(180deg,#f8fafc,#38bdf8)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>
            {P2_STEPS[active - 1]?.title}
          </div>
          <div style={{ fontSize: 16, color: '#93c5fd', maxWidth: 900, textAlign: 'center', lineHeight: 1.6 }}>
            {P2_BIGSCREEN_HINTS[active - 1]}
          </div>
