...
// 从 A01 任务区「点击查看 · 四级核心词汇」打开；按 Day 1-10 分组展示，默认展开第 1 天。
export default function VocabBrowser({ open, onClose, onSendToAI }: { open: boolean; onClose: () => void; onSendToAI?: () => void }) {
    ...
    >
      ...
      >
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 18, fontWeight: 800 }}>四级核心词汇（{VOCAB_TOTAL} 词）</div>
            <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 3 }}>
              10 天 × 每天 {VOCAB_DAILY} 词 · 上下滑动查看全部
            </div>
          </div>
          <button
            className="secondary"
                  ...
                  <div style={{
                    ...
                    {g.words.map((w) => (
                      <div key={w.w} style={{
                        ...
                        <span style={{ color: '#e2e8f0' }}>{w.d}</span>
                      </div>
                    ))}
                  </div>
                )}
