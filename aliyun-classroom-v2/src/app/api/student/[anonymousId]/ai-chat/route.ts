...
export async function POST(req: NextRequest, { params }: { params: { anonymousId: string } }) {
  try {
    ...
    const attachVocab: boolean = !!body.attachVocab;
  ...
  if (!mod || mod.type !== 'ai_task') {
    ...
    if (message) {
      ...
      if (attachVocab) effectiveMessage = `【已附加资料：四级核心词汇 400 词】\n${vocabText()}\n\n${message}`;
      if (!data.firstUserPrompt) data.firstUserPrompt = effectiveMessage;
      data.turns.push({ role: 'user', content: effectiveMessage, at: Date.now() });
      data.usedMaterial = detect(effectiveMessage, data.usedMaterial, MATERIAL_KW) || materialReferenced;
      data.verified = detect(effectiveMessage, data.verified, VERIFY_KW);
      data.modified = detect(effectiveMessage, data.modified, MODIFY_KW);
