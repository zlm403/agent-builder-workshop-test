...
`student/` 是三项目通用的学生端壳：项目切换（项目一/二/三）+ 任务看板（按项目接收老师推送）+ "和老师聊"对话框 + 本课引导 + 我的学习水位（6 大知识点水杯 + AI 分析）。

**不嵌顷悟对话通道**：学员与 AI 的对话在顷悟 APP 内完成（每课一个顷悟应用，对话数据由 `qingwu-dialog-bridge.py` 转发上报）；本页只负责任务、聊天、引导与学习水位展示。
